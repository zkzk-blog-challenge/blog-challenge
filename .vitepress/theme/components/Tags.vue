...
// 1. posts 구조 직접 출력
console.log('[Tags.vue] theme.value:', theme.value)
console.log('[Tags.vue] theme.value.posts:', theme.value.posts)

// 2. initTags 실행 전/후 콘솔
const rawPosts = theme.value.posts
const tagMap = initTags(rawPosts)

const data = computed(() => {
  // computed에서 console 찍으면 reactivity 변화도 다 추적됨!
//   console.log('[Tags.vue][computed] initTags(theme.value.posts)', theme.value.posts)
